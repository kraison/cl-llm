...
### Evidence bundles (`cl-llm/rag` and `cl-llm/rag/eval`)

`cl-llm/rag` fuses dense and sparse retrieval (`rag:fuse`) into one
ordered `bundle` of `evidence` — ranked, attributed to the mode that
produced it, and carrying a non-`NIL` standing so absence always has a
reason. A caller who wants bundles has them already, with no dependency
beyond `cl-llm/rag` itself. `cl-llm/rag/eval` adds four deterministic
